...
If you run the connector with Kubernetes checks enabled (`include_kubernetes=True`), the health checker will always include:

- **Kubernetes Connectivity**: Verifies API access to the cluster (using in-cluster or local kubeconfig)
- **Kubernetes Cluster**: Checks the health of all nodes in the cluster (number of ready nodes, total nodes)

These checks are shown in the health table output, for example:

```
┌─────────────────────────┬──────────────┬────────────────────────────┐
│ Component               │ Status       │ Message                    │
├─────────────────────────┼──────────────┼────────────────────────────┤
│ EDA Connectivity        │ HEALTHY      │ EDA is reachable           │
│ EDA Authentication      │ HEALTHY      │ Authentication successful  │
│ EDA Version             │ HEALTHY      │ Version v25.4.3 is support│
│ Kubernetes Connectivity │ HEALTHY      │ Kubernetes API is reachable│
│ Kubernetes Cluster      │ HEALTHY      │ All 3 nodes ready          │
└─────────────────────────┴──────────────┴────────────────────────────┘
```

If the cluster is not healthy, the status will show DEGRADED or UNHEALTHY with details about node readiness.
